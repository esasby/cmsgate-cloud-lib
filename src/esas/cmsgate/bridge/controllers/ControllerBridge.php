...
class ControllerBridge extends Controller
{
    ...
    public function process() {
        try {
            ...
            $controller = null;
            if (StringUtils::endsWith($request, RedirectServiceBridgeImpl::PATH_CONFIG_LOGIN)) {
                $controller = new ControllerBridgeLogin();
            } elseif (StringUtils::endsWith($request, RedirectServiceBridgeImpl::PATH_CONFIG_LOGOUT)) {
                $controller = new ControllerBridgeLogout();
            } elseif (StringUtils::contains($request, RedirectServiceBridgeImpl::PATH_CONFIG)) {
                $controller = new ControllerBridgeConfig();
            } elseif (StringUtils::contains($request, RedirectServiceBridgeImpl::PATH_CONFIG_SECRET_NEW)) {
